...
def parse_lat_lon(raw_lat, lat_dir, raw_lon, lon_dir):
    try:
        ...
        lat_dir = lat_dir.strip()
        lon_dir = lon_dir.strip()
...
def parse_gpgga(sentence):
    try:
        ...
        if debug: print("GPGGA parts: ", parts)
        if parts[0] != "$GPGGA":
            return None
...
def parse_gprmc(sentence):
    try:
        parts = sentence.split(',')
        if debug: print("GPRMC Parts: ", parts)
...
def parse_gpgll(sentence):
    try:
        parts = sentence.split(',')
        if debug: print("GPGLL Parts:  ", parts)
        if parts[0] != "$GPGLL" or parts[6] != 'A':
            return None
...
# === Read and return one set of coordinates ===
def get_lat_lon():
    if not gps_uart:
        return None
    while True:
        ...
        try:
            ...
            if debug: print("Raw NMEA Sentence:", sentence)
            ...
            if debug: print("Parsed Coordinates:", coords)
